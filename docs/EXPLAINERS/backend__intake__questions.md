...
- **`localized(d, language)`** — returns the language variant, falling back to English (this is
  how any unsupported language degrades safely).
- **`INTAKE_FIELDS`** — the 17 fields (consent gate → identity → complaint loop → history →
  additional). English canonical; **Hindi + Marathi** authored, all `needs_clinical_review=True`.
  Critical fields: chief_complaint, medications, allergies. Red-flag-screen fields:
  associated_symptoms, allergies.
...
## Gotchas / TODOs
- Hindi + Marathi strings are machine-authored — must be clinician-reviewed before real use (§7).
- `localized` falls back to English for any language without a translation.
